...
class RuleMutation(model : Model, verbose: Boolean) : Mutation(model, verbose) {
    ...
    private fun getCandidates() : List<NodeMap> {
        ...
        for (v in ruleConfig.bodyVariables.minus(argumentVariables.toSet())) {
            ...
            if (freshNodeAtomsBody.none { a -> a.containsResource(v) }) {
                if (verbose)
                    println("Variable \"${v.localName}\" does not occur in a positive atom. This violates a requirement of how swrl" +
    ...
    override fun createMutation() {
        ...
        for (a in ruleConfig.head) {
            when (a){
                ...
                else -> if(verbose) println("the type of mutation atom is not supported in the head for atom $a")
            }
        }
    ...
    private fun mimicReplacementMutation(oldNode: RDFNode, newNode: RDFNode) {
        val m = ReplaceNodeWithNodeRules(model, oldNode, newNode, verbose)
        m.applyCopy()
